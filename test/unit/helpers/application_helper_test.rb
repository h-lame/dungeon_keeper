...
class ApplicationHelperTest < ActionView::TestCase
  context "actions_menu" do
    should "wrap each argument in a <li> inside a single <ul> with the class 'actions'" do
    ...
    should "output nothing if no arguments are supplied" do
      assert actions_menu.blank?
    end
  end

  context "yes_or_no" do
    should "return 'yes' if the supplied argument is truthy" do
      [true, '', :a_thing].each do |truthy_value|
        assert_equal 'yes', yes_or_no(truthy_value)
      end
    end

    should "return 'no' if the supplied argument is falsy" do
